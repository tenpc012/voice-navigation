...
window.selectLanguage = function(langCode) {
  ...
  setTimeout(() => {
    ...
    // 定義 21 號之後要呈現的英文字母清單
    const letterPrefixes = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'];

    // 生成 01 ~ 29 景點竹簡
    currentSpotList.forEach((spot, index) => {
      let numStr = "";
      
      // 判斷編號：前 20 個維持 01~20，從第 21 個 (index >= 20) 開始使用字母 A, B, C...
      if (index >= 20) {
        const letterIndex = index - 20;
        // 防護：若超過字母陣列長度，則自動計算 A-Z
        numStr = letterPrefixes[letterIndex] || String.fromCharCode(65 + letterIndex);
      } else {
